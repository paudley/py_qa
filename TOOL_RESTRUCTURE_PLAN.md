...
- Legacy command classes / registry shims were removed in favour of
  data-driven strategies; tests exercise the strategy layer directly.
...
4. **Parity Gate**: Confirm every tool (100% coverage) is defined in the catalog with matching behaviour by running the full test matrix using catalog-backed registry exclusively.
5. **Cutover**: Switch registry to catalog by default, remove the feature flag, and excise legacy registry initialization paths.
6. **Cleanup**: Delete old `builtin_commands_*` definitions, drop version constants (e.g. `HADOLINT_VERSION_DEFAULT`), update docs/CLI help, and remove deprecated suppressions.
