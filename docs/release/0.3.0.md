...
* Renamed the published distribution to `pyqa_lint` to avoid the existing
  `py_qa` package on PyPI. The import namespace remains `pyqa`, so existing code
  can continue to `import pyqa` after installing the new package.
