...
def normalize_cli_values(values: Sequence[str] | None) -> tuple[str, ...]:
    ...
    Returns:
        tuple[str, ...]: Tuple of stripped, non-empty values.
    """
...
def _build_clean_path_options(
    ...
    Args:
        ...
        include_tree: Optional repeatable tree option values.

    Returns:
        tuple[tuple[str, ...], tuple[str, ...]]: Normalised pattern and tree tuples.
    """
...
def build_clean_options(
    ...
    Args:
        ...
        path_args: Normalised pattern and tree values.
        dry_run: Flag indicating whether sparkly-clean should run in dry-run mode.
        emoji: Flag controlling emoji usage in CLI output.

    Returns:
        CleanCLIOptions: Structured CLI options for sparkly-clean execution.
    """
