...
def _normalise_files(paths: list[Path] | None) -> tuple[Path, ...]:
    """Return normalised file paths expanded relative to the user.

    Args:
        paths: Optional list of user-supplied file paths.

    Returns:
        tuple[Path, ...]: Tuple of expanded file paths.
    """
...
def build_security_options(
    ...
    Args:
        files: Optional list of files provided via CLI arguments.
        root: Project root directory used for relative resolution.
        staged: Flag indicating whether staged files should be scanned.
        no_bandit: Flag disabling Bandit analysis when ``True``.
        no_emoji: Flag disabling emoji output when ``True``.

    Returns:
        SecurityCLIOptions: Structured CLI options for the security scan.
    """
