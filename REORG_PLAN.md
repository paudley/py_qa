...
1. **Inventory & ownership**
   - Generate a dependency graph (`pydeps`, `pyan`, or custom script) to confirm
     existing module coupling.
   - Mark files that will move to the new packages; capture outstanding local
     patches that could conflict.
2. **Automation scaffolding**
   - Add architectural tests (import lints) to enforce the new package
     boundaries as they are created.
   - Extend CI to run contract suites (type checking, pytest selection) against
     both current and in-progress layouts.
3. **Entry-point skeletons**
   - Define plugin group names in `pyproject.toml` (`pyqa.catalog.plugins`,
     `pyqa.cli.plugins`, `pyqa.diagnostics.plugins`).
   - Create placeholder registries in `interfaces/` that will hydrate the entry
     points once implementations move.

## Phase 1 – Interfaces & Core Runtime (2 sprints)
1. **Introduce `interfaces/` package**
   - Create domain files (`analysis.py`, `catalog.py`, `cli.py`, `config.py`,
     `core.py`, `discovery.py`, `environment.py`, `orchestration.py`,
     `reporting.py`).
   - Lift existing `Protocol` definitions into the package, adding invariants
     and docstrings.
2. **Core runtime refresh**
   - Stand up `core/runtime/di.py` with a lightweight service container,
     default factories (console/logger/cache), and tests using stub bindings.
   - Move logging helpers into `core/logging/` with shared formatters and Rich
     adapters wrapped behind interfaces.
   - Add `cache/in_memory.py` with TTL cache primitives; update call sites in
     modules that currently rely on `functools.cache`.
3. **Gateway tests**
